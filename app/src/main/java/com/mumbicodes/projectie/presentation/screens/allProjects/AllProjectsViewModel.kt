...
import com.mumbicodes.projectie.domain.model.ProjectJson
...
class AllProjectsViewModel @Inject constructor(
        ...
        viewModelScope.launch {
            when (val results = projectsUseCases.getProjectsUseCase()) {
                is DataResult.Success -> {
                    results.data.collectLatest { projects ->
                        if (projects.isEmpty()) {
                            insertMockProjects()
                        }
                    }
    ...
    private fun insertMockProjects() {

        viewModelScope.launch {

            val inputStream = appContext.assets.open("projects.json")

            val reader = InputStreamReader(inputStream)

            val projectType = object : TypeToken<List<ProjectJson>>() {}.type

            val projects: List<ProjectJson> =
                Gson().fromJson(reader, projectType)
